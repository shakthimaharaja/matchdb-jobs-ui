...
const VendorsManagementView: React.FC<Props> = () => {
  const [statusFilter, setStatusFilter] = useState("");
  const [search, setSearch] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingVendor, setEditingVendor] =
    useState<VendorCompanyEnhanced | null>(null);
  ...
  const columns: DataTableColumn<VendorCompanyEnhanced>[] = useMemo(
    () => [
      ...
      { key: "category", header: "Category" },
      {
        key: "paymentTerms",
        header: "Terms",
        render: (v) => `Net ${v.paymentTerms}`,
      },
      {
        key: "status",
        header: "Status",
        render: (v) => (
          <span style={{ color: STATUS_COLORS[v.status], fontWeight: 600 }}>
            {v.status}
          </span>
        ),
      },
    ...
    <>
      ...
      {showForm && (
        ...
        >
          <button
            type="button"
            aria-label="Close vendor form dialog"
            onClick={() => {
              setShowForm(false);
              setEditingVendor(null);
            }}
