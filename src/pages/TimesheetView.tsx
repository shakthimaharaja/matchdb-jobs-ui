...
const TimesheetView: React.FC<Props> = ({
  candidateName = "",
  ...
  const [entries, setEntries] = useState<
    Record<string, { hoursWorked: string; notes: string }>
  >(() =>
    Object.fromEntries(
      weekDates.map((wd) => [wd.date, { hoursWorked: "8", notes: "" }]),
    ),
  );
  const [saveMsg, setSaveMsg] = useState<string | null>(null);
  const [saveErr, setSaveErr] = useState<string | null>(null);
  const [submitConfirm, setSubmitConfirm] = useState(false);
  const [approverNotesModal, setApproverNotesModal] = useState<string | null>(
    null,
  );
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetTimesheetsQuery({ page, limit: 25 });
  ...
  const existingDraft = useMemo(
    () =>
      timesheets.find(
        (t) =>
          t.weekStart.slice(0, 10) ===
            currentWeekStart.toISOString().slice(0, 10) && t.status === "draft",
      ),
  ...
  return (
    <div className="matchdb-page">
      ...
      <div
        ...
        style={{ flex: "0 0 auto", marginBottom: 4 }}
      >
        <div className="matchdb-panel-title">
          ...
          <div
            className="matchdb-panel-title-right"
        ...
        {isCurrentWeekAlreadySubmitted ? (
          ...
          >
            This week&apos;s timesheet is{" "}
            <strong>{currentWeekSheet.status}</strong>
            {currentWeekSheet.approvedAt &&
              ` on ${fmtDate(currentWeekSheet.approvedAt)}`}
            .
            {(currentWeekSheet.status === "submitted" ||
                ...
                <tbody>
                  {weekDates.map((wd, i) => (
                    ...
                    >
                      ...
                      >
                        {new Date(wd.date + "T00:00:00Z").toLocaleDateString(
                          "en-US",
                          {
                            month: "short",
                            day: "numeric",
          ...
          >
            ...
            <div style={{ padding: "14px 16px", fontSize: 13 }}>
              <p style={{ margin: "0 0 10px" }}>
                ...
                <strong>{fmtWeek(currentWeekStart.toISOString())}</strong>?
              </p>
              <p style={{ margin: "0 0 14px", fontSize: 12, color: "#555" }}>
                Total: <strong>{totalHours.toFixed(1)} hours</strong>. Once
                submitted, your employer will review and approve it. You
